...
def get_ibis_query(DATE="1994-01-01", DISCOUNT=0.06, QUANTITY=24):
  ...
  q = lineitem
  discount_min = int(round(DISCOUNT - 0.01, 2))
  discount_max = int(round(DISCOUNT + 0.01, 2))
  q = q.filter([
      q.l_shipdate >= ibis.literal(DATE, "timestamp"),
      q.l_shipdate < ibis.literal(add_date(DATE, dy=1), "timestamp"),
      q.l_discount.between(ibis.literal(discount_min, "decimal(32,2)"),
                           ibis.literal(discount_max, "decimal(32,2)")),
