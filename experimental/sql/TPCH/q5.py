...
def get_ibis_query(NAME="ASIA", DATE="1994-01-01"):
  ...
  q = q.filter([
      q.r_name == NAME, q.o_orderdate >= ibis.literal(DATE, "timestamp"),
      q.o_orderdate < ibis.literal(add_date(DATE, dy=1), "timestamp")
  ])
  revexpr = q.l_extendedprice * (ibis.literal(1, "int64") - q.l_discount)
  proj = q.projection([q.n_name, (revexpr).name('revenue')])
